module SSAmechanistic

export SSAmech,mean,var

# define the propensity function.
function propensity(n,pars)
    # Reaction rates
    a = pars[1]; ap = pars[2]; b = pars[3]; bp = pars[4];
    c = pars[5]; d = pars[6]; f = pars[7];
...
function SSAmech(S_time::Int, pars::Array{Float64,1}, τ::Float64, tol_time::Float64, sp::Float64)
    ...
    S_mat[6,1:N] = [0,1,1,0];
    S_mat[7,1:N] = [0,0,0,-1];
    S_mat[8,1:N] = [0,0,-1,1];# this is the delayed reaction.
    # Define reactants trjatory vector
    n = zeros(N,S_time,Int(floor(tol_time/sp)));

    for sim in 1:S_time
        n_temp = [0;0;0;0]; # gene starts in U** state with zero nascent or mature.
        T = 0;
        delay_times = [];
